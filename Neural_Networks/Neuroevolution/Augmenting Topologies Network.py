...
def matrix_init_params(env, layers, population):
    paramsList = []
    for p in range(population):
        paramsList.append(init_params(env, layers))
        #for key in params:
         #   print(key, params[key].shape)
    return paramsList

def forward_prop(state, params, outputType):
    activations = {}
    length = (len(params) // 2)
    output = []
...
def main():
   ...
   #print(a)
    env = load_environment(CartPole-v1) #CartPole-v1 #BipedalWalker-v2 #LunarLander-v2 #MountainCar-v0
